...
for index, mol_batch in enumerate(tqdm(iterator, total=num_batches,
                                       unit="batch")
                                  ):
    inds = np.arange(index * hdf_chunk_size,
                     index * hdf_chunk_size + len(mol_batch))
    ...
    decoy_fps = utils.simple_parallel(offset_lens, get_decoy_fps,
                                      max_cpu=num_workers,
                                      timeout=60 * 3,)
    ...
    def get_weights(input_tuple, epsilon=1e-12):
        ...
        padded_decoys = [np.vstack([decoys, np.zeros((padding, fp_shape))])
        ...
        sample_weights = [
            np.argsort(i[j]) + offset for i, j, offset in zip(sample_weights,
                                                              valid_decoys,
                                                              offsets
                                                              )
        ]
        ...
        true_inds = [
            ...
            for i, j, offset in zip(tanimoto_sim,
                                    valid_decoys,
                                    offsets
                                    )
        ]
    ...
    # parallel
    iterator = list(utils.batches(zip(fp_lengths, true_fps, decoy_fps,
                                      offsets
                                      ),
                                  parallel_chunk_size))
    # Apply this in parallel
    # 3D vector of vector of vectors
    sample_weights = utils.simple_parallel(iterator, get_weights,
                                           max_cpu=num_workers,
                                           timeout=60 * 10,)
...
for ind, k in enumerate(inchikey_list):
    out_dict[k] = {"rankings":  sample_weights[ind].tolist(),
